/* Deauville 2026 — V1.6.1
   V1.6.1 : identité Compétition dorée pour tout le programme issu du PDF Jury,
...
function render(){
 ...
 <div class="topline"><div><div class="brand">DEAUVILLE <span class="version">V1.6.1</span></div><div class="sub">FESTIVAL DU CINÉMA AMÉRICAIN · 2026</div></div></div>
...
function fixedRow(x){const jury=isJuryItem(x);return `<div class="event ${jury?"juryActivity":""}" onclick='openFixed(${JSON.stringify(x)})'><div class="time">${displayTime(x.s)}${x.e?"–"+displayTime(x.e):""}</div><div class="content"><div class="title">${jury?"🏆":""} ${x.title}</div><div class="meta"><span>${x.place}</span><span>·</span>${jury?competitionBadge():`<span class="tag">${x.cat}</span>`}</div></div></div>`}
...
function openFixed(x){
 ...
 open(`<div class="section">${isPlanned?"MON PLANNING":(isJuryItem(x)?"🏆 COMPÉTITION":"JURY · OBLIGATOIRE")}</div><h2>${x.title}</h2><div class="info">📅 ${dateLabel(x.date)}<br>🕘 ${displayTime(x.s)}${x.e?"–"+displayTime(x.e):""}<br>📍 ${x.place}<br>🏷️ ${x.cat}</div>${actions}<button class="btn" onclick='openRoute(${JSON.stringify(x.place)})'>🗺️ Itinéraire</button>`)
}
