...
const ALIAS_GROUPS={
  gremlins:["gremlins, l’amérique parasitée","gremlins, an american nightmare","american nightmare"],
};
...
function plannedDay(){return planned.filter(x=>x.date===DAYS[day])}
...
function compatible(p,w){
  ...
  return s>=tm(w.s)&&e<=tm(w.e)&&!juryDay().some(j=>s<tm(j.e)&&e>tm(j.s))&&!plannedDay().some(j=>s<tm(j.e)&&e>tm(j.s));
}
...
function render(){
 ...
 <div class="topline"><div><div class="brand">DEAUVILLE <span class="version">V1.6.2</span></div><div class="sub">FESTIVAL DU CINÉMA AMÉRICAIN · 2026</div></div></div>
...
function programmeRow(p){
 const i=PROGRAM.indexOf(p), alias=sameWork(p.title,"Gremlins, l’Amérique parasitée")?`<span class="tag">American Nightmare</span>`:"", jury=isJuryProgram(p);
...
function pickDate(){open(`<div class="section">CHOISIR UNE DATE</div><h2>Mon festival</h2><div class="datepick">${DAYS.map((d,i)=>`<button class="${i===day?"sel":""}" onclick="goToDay(${i})">${dateLabel(d)}</button>`).join("")}</div>`)}
...
function openFixed(x){
 ...
 const wishBack=isPlanned&&wishes.includes(workIndex(PROGRAM.findIndex(p=>sameWork(p.title,x.title))));
 ...
 open(`<div class="section">${isPlanned?"MON PLANNING":(isJuryItem(x)?"🏆 JURY":"JURY · OBLIGATOIRE")}</div><h2>${x.title}</h2><div class="info">📅 ${dateLabel(x.date)}<br>🕘 ${displayTime(x.s)}${x.e?"–"+displayTime(x.e):""}<br>📍 ${x.place}<br>🏷️ ${x.cat}</div>${actions}<button class="btn" onclick='openRoute(${JSON.stringify(x.place)})'>🗺️ Itinéraire</button>`)
}
...
function agendaSessionsForWork(title){
 const rows=PROGRAM.filter(p=>sameWork(p.title,title)).sort((a,b)=>a.date.localeCompare(b.date)||tm(a.time)-tm(b.time));
 return rows.map(p=>{const saved=day;const di=DAYS.indexOf(p.date);day=di>=0?di:saved;const end=calcEnd(p);const known=end!==null;const ok=known&&!juryDay().some(j=>tm(p.time)<tm(j.e)&&end>tm(j.s))&&!plannedDay().some(j=>tm(p.time)<tm(j.e)&&end>tm(j.s));day=saved;return {...p,agendaCompatible:ok,knownEnd:known};});
}
...
function openProgramById(i){
 const p=PROGRAM[i];if(!p)return;const key=workIndex(i),w=wishes.includes(key),v=seen.includes(key),pl=planned.some(x=>sameWork(x.title,p.title)&&x.date===p.date);
 const sessions=agendaSessionsForWork(p.title),firstCompatible=sessions.find(x=>x.agendaCompatible),others=sessions.filter(x=>!firstCompatible||x.date!==firstCompatible.date||x.time!==firstCompatible.time||x.place!==firstCompatible.place);
 ...
 const jury=isJuryProgram(p),category=jury?competitionBadge():`<span class="tag">${p.cat}</span>`;
 open(`<div class="section">${p.typ.toUpperCase()} · ${p.cat}</div><h2>${p.title}</h2>${sameWork(p.title,"Gremlins, l’Amérique parasitée")?'<div class="info">Titre alternatif : <b>American Nightmare</b>. Cette œuvre est regroupée avec « Gremlins, l’Amérique parasitée » dans les envies, vus et recherches.</div>':""}<div class="info">📅 ${dateLabel(p.date)}<br>🕘 ${p.time}${p.end?"–"+p.end:""}<br>📍 ${p.place}<br>${p.duration?"⏱ "+fmt(p.duration)+"<br>":""}${p.person&&p.person!=="—"?"👤 "+p.person+"<br>":""}${category}</div>${sessionBlock}<p>${p.typ==="Film"?"Fiche film prête à être enrichie avec synopsis et durée officielles.":"Événement du programme officiel."}</p><button class="btn" onclick="toggleWish(${i})">${w?"⭐ Retirer des envies":"⭐ Ajouter aux envies"}</button><button class="btn primary" onclick="addPlan(${i})">${pl?"📅 Déjà dans mon planning":"📅 Ajouter à mon planning"}</button><button class="btn" onclick="markSeen(${i})">${v?"👀 Déjà marqué comme vu":"👀 Marquer comme vu"}</button><button class="btn" onclick="addNote(${i})">📝 Ajouter une note</button><button class="btn" onclick='openRoute(${JSON.stringify(p.place)})'>🗺️ Itinéraire</button>`);
}
...
function addPlan(i){
 const p=PROGRAM[i];
 if(planned.some(x=>sameWork(x.title,p.title)&&x.date===p.date)){show("📅 Déjà dans ton planning");return}
