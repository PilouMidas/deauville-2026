/* Deauville 2026 · V2.2.23 — film linked to a planned event */
(function(){
  'use strict';
  ...
  function exactFilm(a,b){
    const aa=normFilm(a),bb=normFilm(b);
    return !!aa&&!!bb&&aa===bb;
  }
  function jurySessions(){
  ...
  function eventContainsFilm(event,s){
    const target=normFilm(s);
    if(!target)return false;
    const raw=titleOf(event).trim();
    if(!raw)return false;
  ...
  function plannedEventForFilm(s){
    ...
    return plan.find(p=>eventContainsFilm(p,s))||null;
  }
  ...
  function personalEventForFilm(s){
    return plannedEventForFilm(s)||plannedJuryEventForFilm(s)||null;
  }
  ...
  window.sessionStatusHtml=function(s){
    ...
    if(status==='JURY · OBLIGATOIRE · DANS MON PLANNING')return '<div class="wishStatus planned">📅 '+status+'</div>';
    if(status==='DANS MON PLANNING')return '<div class="wishStatus planned">✓ '+status+'</div>';
    if(status==='NON COMPATIBLE')return '<div class="wishStatus bad">⚠️ '+status+'</div>';
    return '<div class="wishStatus good">✓ '+status+'</div>';
  };
  window.compatibilityLabel=function(s){
    const status=statusFor(s);
    const cls=status==='NON COMPATIBLE'?'bad':'planned';
    return '<span class="status '+cls+'">'+(status==='NON COMPATIBLE'?'⚠️ ':'✓ ')+status+'</span>';
  };
  ...
  if(typeof oldOpenFree==='function')window.openFree=function(start,end){
    ...
    const cards=opts.length?opts.map(o=>{
      const status=statusFor(o),cls=status==='JURY · OBLIGATOIRE · DANS MON PLANNING'?'jury':status==='DANS MON PLANNING'?'planned':status==='NON COMPATIBLE'?'bad':'good';
