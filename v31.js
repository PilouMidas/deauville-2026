/* Deauville 2026 · V3.0.1 — accès explicite aux fiches films */
(function(){
  'use strict';

  const VERSION_LABEL='V3.0.1';
  ...
  function addFilmLink(s){
    ...
    const existing=sheet.querySelector('.v3FilmLink');
    if(existing)existing.remove();

    const row=document.createElement('div');
    row.className='v3FilmAction';
    const button=document.createElement('button');
    button.type='button';
    button.className='v3FilmLink';
    button.textContent='Voir la fiche du film';
  ...
  function decorateSessionSheet(s){
    ...
    function run(){
      attempts++;
      const linked=addFilmLink(s);
      addNotes(s);
      if(!linked&&attempts<8)requestAnimationFrame(run);
    }
  ...
  style.textContent=`
    ...
    .v3FilmAction{margin:6px 0 0}
    .v3FilmLink{display:inline-flex;align-items:center;padding:7px 11px;border:1px solid rgba(201,162,39,.7);border-radius:999px;background:rgba(201,162,39,.1);color:inherit;font-size:12px;font-weight:700;cursor:pointer;white-space:nowrap}
    .v3FilmLink:hover{background:rgba(201,162,39,.18)}
